...
//will not delete in database but will in ui
    ...
    private boolean checkIfUserCanBeDeleted(User user){
        switch (user.getRole().getDescription()){
            ...
            case "Manager":
                List<ProjectDTO> projectDTOList = projectService.listAlNonCompletedByAssignedManager(userMapper.convertToDto(user));
           return projectDTOList.size()==0;


            case "Employee":
                List<TaskDTO> taskDTOList = taskService.listAlNonCompletedByAssignedEmployee(userMapper.convertToDto(user));
